...
```{r}
plot(x = pca_iris$x[,1], y = pca_iris$x[,2])
```
...
```{r}
plot(x = pca_iris$x[,1], y = pca_iris$x[,2], col = iris$Species)
```
...
```{r}
pca_iris_sum <- summary(pca_iris)
explained_variance_ratio <- pca_iris_sum$importance[2,]
...
```{r}
# single
hclust_iris_single <- hclust(dist_iris)
...
```{r}
# using table()
table(iris_hclust_ward_labels,
      iris$Species)
```

```{r, fig.height=7, fig.width=7}
par(mfrow = c(2, 2))
plot(x = pca_iris$x[,1], y = pca_iris$x[,2], col = iris_hclust_ward_labels)
text(x = pca_iris$x[,1], y = pca_iris$x[,2], labels = iris_hclust_ward_labels, pos = 3, cex = 0.8, col = iris_hclust_ward_labels)
plot(x = pca_iris$x[,1], y = pca_iris$x[,2], col = iris_hclust_complete_labels )
text(x = pca_iris$x[,1], y = pca_iris$x[,2], labels = iris_hclust_complete_labels, pos = 3, cex = 0.8, col = iris_hclust_complete_labels)
plot(x = pca_iris$x[,1], y = pca_iris$x[,2], col = iris_hclust_average_labels )
text(x = pca_iris$x[,1], y = pca_iris$x[,2], labels = iris_hclust_average_labels, pos = 3, cex = 0.8, col = iris_hclust_average_labels)
plot(x = pca_iris$x[,1], y = pca_iris$x[,2], col = iris_hclust_single_labels )
text(x = pca_iris$x[,1], y = pca_iris$x[,2], labels = iris_hclust_single_labels, pos = 3, cex = 0.8, col = iris_hclust_single_labels)
par(mfrow = c(1, 1))
```
