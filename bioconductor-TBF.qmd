...
```{r}
#| eval: false # show the code but not execute 
...
countPattern(Ebox, chrY, max.mismatch = 1, fixed = FALSE, algorithm = "auto")
matchPattern(Ebox, chrY, max.mismatch = 0, fixed = FALSE, algorithm = )
```
...
```{r}
gtf_ACTB <- S4Vectors::subset(gtf_data, gene_name == "ACTB")
gtf_ACTB
```
...
```{r}
gtf_ACTB_exons <- S4Vectors::subset(gtf_ACTB, type == "exon")
gtf_ACTB_exons
gtf_ACTB_transcripts <- S4Vectors::split(x = gtf_ACTB_exons, f = gtf_ACTB_exons$transcript_id)
...
```{r}
keytypes(org.Hs.eg.db) ## keytypes vs keys?
```
...
```{r}
mapIds(org.Hs.eg.db, keys = human_symbols, column = "ENSEMBL", keytype = ) 
```
